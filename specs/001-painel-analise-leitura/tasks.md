...
- [ ] T001 Inicializar projeto Next.js com App Router e React em `/` (`package.json`, `next.config.ts`)
  - ✓ `npm run dev` sobe e serve uma rota
- [ ] T002 Configurar TypeScript strict em `tsconfig.json` (`strict`, `noUncheckedIndexedAccess`, `noImplicitOverride`, `exactOptionalPropertyTypes`)
  - ✓ `npm run typecheck` passa sem `any` implícito
- [ ] T003 [P] Configurar ESLint em `eslint.config.mjs`, incluindo regra que proíbe `src/modules/*/domain/**` de importar `react`, `next` ou `@prisma/client`
  - ✓ import proibido em `domain/` falha o lint (Const. VI)
- [ ] T004 [P] Configurar Prettier e EditorConfig em `.prettierrc`, `.editorconfig`
- [ ] T005 Criar a árvore de diretórios de `plan.md` em `src/`, `prisma/`, `tests/`
  - ✓ os treze módulos existem com as quatro camadas
- [ ] T006 [P] Criar `docker-compose.yml` com PostgreSQL 16 e volume nomeado
  - ✓ `docker compose up -d db` responde a `pg_isready`
- [ ] T007 [P] Criar `.env.example` e validação de ambiente com Zod em `src/lib/env.ts` (`DATABASE_URL`, `SESSION_SECRET`, `IMPORT_STORAGE_DIR`, `IMPORT_MAX_FILE_SIZE_MB`)
  - ✓ variável ausente derruba o boot com mensagem clara
- [ ] T008 Inicializar Prisma em `prisma/schema.prisma` com datasource PostgreSQL
- [ ] T009 Escrever o schema completo em `prisma/schema.prisma`: enums e os 16 modelos de [data-model.md](data-model.md)
  - ✓ `acertos`/`itensPossiveis` são `Int?`; `percentual` é `Decimal @db.Decimal(7,4)`; **nenhum `Float` no arquivo**
- [ ] T010 Gerar a migration inicial em `prisma/migrations/`
  - ✓ `npm run db:migrate` aplica em banco vazio sem erro
- [ ] T011 Adicionar CHECK constraints por SQL na migration: `acertos IS NULL OR (acertos >= 0 AND itens_possiveis > 0 AND acertos <= itens_possiveis)`
  - ✓ `INSERT` violando a regra é rejeitado pelo banco (FR-032)
- [ ] T012 Adicionar os índices compostos de [data-model.md](data-model.md) na migration em `prisma/migrations/`
- [ ] T013 Criar o cliente Prisma singleton em `src/server/prisma.ts`
- [ ] T014 Escrever o seed do catálogo das 12 habilidades em `prisma/seed.ts`, com códigos e descrições do PRD §4
  - ✓ 12 registros em `Skill`; **nenhuma quantidade de itens é semeada** (FR-016)
- [ ] T015 Estender o seed com `AnalyticalSettings` versão 1 (60/80, baixo rendimento = Defasagem), um usuário ADMIN e uma escola de demonstração
- [ ] T016 Declarar os scripts npm em `package.json`: `dev`, `lint`, `typecheck`, `test`, `test:regression`, `test:e2e`, `build`, `db:migrate`, `db:seed`
...
- [ ] T017 [US3] Implementar hash e verificação de senha com argon2id em `src/modules/auth/domain/password.ts`
  - ✓ teste unitário: hash não é reversível, verificação aceita a senha correta e rejeita a errada
- [ ] T018 [US3] Implementar criação, leitura, renovação e revogação de sessão em `src/modules/auth/infra/session-repository.ts`
  - ✓ expiração absoluta e por inatividade; revogar é `DELETE` (R-002)
- [ ] T019 [US3] Implementar `signIn` em `src/modules/auth/application/sign-in.ts`
  - ✓ resposta idêntica para e-mail inexistente e senha errada; sessão rotacionada na autenticação
- [ ] T020 [US3] Implementar `signOut` em `src/modules/auth/application/sign-out.ts`
- [ ] T021 [US3] Implementar `resolveAuthContext` a partir do cookie em `src/server/auth-context.ts`
  - ✓ cookie `httpOnly`, `Secure`, `SameSite=Lax`
- [ ] T022 [US3] Implementar `resolveAllowedSchoolIds` — **chokepoint único de autorização** — em `src/server/authorization.ts`
  - ✓ deriva de `UserSchool`; ADMIN recebe todas; nenhum caminho aceita `schoolId` do cliente como autorização (R-003)
- [ ] T023 [US3] Implementar os guardas `requireUser` e `requireRole` em `src/server/authorization.ts`
- [ ] T024 [US3] Implementar o middleware de rota autenticada em `src/middleware.ts`
  - ✓ rota de dado nominal sem sessão redireciona para `/entrar`
- [ ] T025 [US3] Implementar a tela de login em `src/app/(public)/entrar/page.tsx`
- [ ] T026 [US3] Implementar CRUD de usuários (ADMIN) em `src/modules/users/application/` e `src/app/(app)/usuarios/`
- [ ] T027 [US3] Implementar vínculo e desvínculo usuário–escola em `src/modules/users/application/link-school.ts`
- [ ] T189 [US3] Implementar a permissão `canAccessNominalData` no cadastro de usuários em `src/modules/users/application/set-nominal-permission.ts` (FR-007)
  - ✓ padrão na criação: concedida a ADMIN e ESCOLA, negada a ANALISTA; alterável pelo ADMIN e registrada em auditoria
- [ ] T190 [US3] Implementar a supressão de dados nominais na camada de consulta em `src/server/nominal-data.ts` (FR-007a)
...
- [ ] T028 [P] [US3] Escrever os esquemas Zod de auth e users em `src/modules/auth/schemas/` e `src/modules/users/schemas/`
- [ ] T029 [P] [US3] Implementar o logger com guarda de PII em `src/server/logger.ts`
  - ✓ campo de lista negra (`nome`, `nomeOriginal`, `estudante`) derruba o processo em desenvolvimento (FR-009, R-013)
- [ ] T030 [US3] Implementar o mapeamento de códigos de resposta em `src/server/http-errors.ts`
  - ✓ recurso fora de escopo retorna **404, nunca 403** (contrato http-api)
- [ ] T031 [P] [US3] Testes unitários de senha e sessão em `tests/unit/auth/`
- [ ] T032 [US3] Escrever a **matriz de autorização** em `tests/integration/authorization.test.ts`
...
- [ ] T033 [P] Implementar CRUD de escolas em `src/modules/schools/` e `src/app/(app)/escolas/`
- [ ] T034 [P] Implementar CRUD de avaliações em `src/modules/assessments/` e `src/app/(app)/avaliacoes/`
- [ ] T035 [P] Implementar CRUD de turmas em `src/modules/classes/`
  - ✓ `externalCode` gravado já normalizado (FR-033)
- [ ] T191 Implementar a proteção contra exclusão de avaliação, escola ou habilidade com resultados vinculados em `src/modules/assessments/application/delete-guard.ts` (FR-018)
...
- [ ] T036 [P] Implementar leitura do catálogo de habilidades em `src/modules/skills/application/list-skills.ts`
- [ ] T037 Implementar a geração de `uniqueCode` em `src/modules/students/domain/unique-code.ts`
  - ✓ aleatório, não derivado de dado pessoal, alfabeto sem `0/O` e `1/I` (FR-131); teste de colisão em 10⁵ gerações
- [ ] T038 [US10] Implementar cadastro individual de estudante em `src/modules/students/application/create-student.ts`
  - ✓ atribui `uniqueCode` na criação (FR-169); grava `AuditLog` `STUDENT_CREATE` (FR-136)
- [ ] T039 [US10] Implementar edição cadastral de estudante em `src/modules/students/application/update-student.ts`
  - ✓ altera nome, turma e escola; **não toca em nenhum resultado** (FR-178)
- [ ] T040 [US10] Implementar importação de nominata em lote em `src/modules/students/application/import-roster.ts`
  - ✓ reusa as mesmas regras de normalização e severidade da importação de resultados (FR-170); depende de T053 e T054
- [ ] T041 [US10] Implementar exportação da nominata com códigos em `src/app/api/students/roster/route.ts`
  - ✓ respeita escopo e autorização nominal (FR-174)
- [ ] T042 [P] [US10] Implementar busca de estudante por `uniqueCode` em `src/modules/students/application/find-by-code.ts` (FR-133)
- [ ] T043 [US10] Implementar as telas de cadastro e listagem de estudantes em `src/app/(app)/estudantes/`
  - ✓ exibe `uniqueCode`; exibe o **nome original**, nunca o normalizado (FR-132, FR-134)
- [ ] T044 Implementar leitura da versão vigente de `AnalyticalSettings` em `src/modules/settings/application/get-current-settings.ts`
  - ✓ maior `effectiveFrom` não futura; nenhum limite fixo em código (FR-111)
- [ ] T045 [P] Escrever os esquemas Zod dos cadastros em `src/modules/{schools,assessments,classes,students}/schemas/`
- [ ] T046 [P] Testes de integração dos cadastros em `tests/integration/registrations.test.ts`
- [ ] T047 [US10] Teste de integração: dois homônimos coexistem na mesma turma com códigos distintos, em `tests/integration/homonyms.test.ts` (FR-175)
- [ ] T048 [US10] Teste de integração: 111 estudantes por nominata recebem 111 códigos distintos, em `tests/integration/roster-import.test.ts` (SC-021)
...
- [ ] T049 [P] Implementar `toPercent` e `formatPercent` com `Decimal` em `src/lib/decimal.ts`
  - ✓ arredondamento **apenas** em `formatPercent`; ausência devolve `—`, nunca `0%` (FR-063, FR-031)
- [ ] T050 [P] [US1] Implementar `parseSkillResult` em `src/modules/imports/domain/parse-skill-result.ts`
...
- [ ] T051 [P] [US1] Testes unitários de `parseSkillResult` em `tests/unit/domain/parse-skill-result.test.ts`
  - ✓ um caso por linha da tabela do contrato, incluindo a distinção ausência × inválido
- [ ] T052 [P] [US1] Implementar `normalizeStudentName` em `src/modules/students/domain/normalize-name.ts`
  - ✓ trim, colapso de espaços, maiúsculas, remoção de diacríticos; **não altera o nome original** (FR-034)
- [ ] T053 [P] [US1] Implementar `normalizeClassCode` em `src/modules/classes/domain/normalize-class-code.ts`
  - ✓ remove **apenas** espaços das extremidades; preserva caixa e conteúdo interno (FR-033)
- [ ] T054 [P] [US1] Testes unitários de normalização em `tests/unit/domain/normalization.test.ts`
  - ✓ inclui o código real `" 8npu2dd9128c "` e nomes com acento, caixa mista e espaços múltiplos
- [ ] T055 [P] [US1] Implementar `validateSkillDenominators` em `src/modules/imports/domain/validate-denominators.ts`
  - ✓ denominador predominante; empate adota o **maior** e marca `tiebreak` (FR-160); lista as linhas divergentes
- [ ] T056 [P] [US1] Testes unitários de denominadores em `tests/unit/domain/validate-denominators.test.ts`
  - ✓ conjunto uniforme, conjunto divergente e conjunto com empate
- [ ] T057 [US1] Implementar a detecção de BOM e de codificação em `src/modules/imports/infra/encoding.ts`
  - ✓ BOM removido **antes** da normalização de cabeçalhos; acentos preservados (FR-023, FR-024)
- [ ] T058 [US1] Implementar a detecção de separador CSV em `src/modules/imports/infra/delimiter.ts`
  - ✓ escolhe entre `;`, `,` e tabulação; sobreposição manual disponível (FR-021)
- [ ] T059 [US1] Implementar o leitor de CSV com `csv-parse` e `iconv-lite` em `src/modules/imports/infra/csv-reader.ts`
  - ✓ processa o arquivo real com `;` e UTF-8 com BOM sem ajuste manual (FR-022)
- [ ] T060 [US1] Implementar o leitor de XLSX e XLS com SheetJS em `src/modules/imports/infra/xlsx-reader.ts`
  - ✓ instalado do registro oficial do SheetJS com versão fixada (R-005); primeira aba por padrão, com escolha manual
- [ ] T061 [US1] Implementar a identificação de formato por extensão, MIME e assinatura de conteúdo em `src/modules/imports/infra/format-detector.ts`
- [ ] T062 [US1] Implementar a normalização de cabeçalhos e o reconhecimento de `H 01` … `H 12` em `src/modules/imports/domain/header-mapping.ts`
  - ✓ reconhece `H 01`, `H01`, `H_01` e `H 01 (2EF08_P)` (FR-026)
- [ ] T063 [P] [US1] Testes unitários de mapeamento de cabeçalhos em `tests/unit/domain/header-mapping.test.ts`
- [ ] T064 [US1] Testes de integração dos leitores em `tests/integration/readers.test.ts`
...
- [ ] T065 [US1] Implementar upload em `src/app/api/imports/route.ts`
  - ✓ valida extensão, MIME e tamanho contra `IMPORT_MAX_FILE_SIZE_MB`; retorna 413 acima do limite
- [ ] T066 [US1] Implementar o cálculo de SHA-256 e a gravação no volume em `src/modules/imports/infra/file-storage.ts`
  - ✓ arquivo nomeado pelo hash; caminho e hash em `Import` (R-006, FR-038)
- [ ] T067 [US1] Implementar a criação do registro `Import` com avaliação e escola em `src/modules/imports/application/create-import.ts`
  - ✓ `assessmentId` e `schoolId` validados contra `allowedSchoolIds`; escola obrigatória quando ausente do arquivo (FR-028)
- [ ] T068 [US1] Implementar a detecção de arquivo já importado por hash em `src/modules/imports/application/detect-duplicate-file.ts`
  - ✓ emite `FILE_ALREADY_IMPORTED` como `WARNING`
- [ ] T069 [US1] Implementar a máquina de estados de `Import` em `src/modules/imports/domain/import-status.ts`
  - ✓ apenas as transições do diagrama de [data-model.md](data-model.md) são aceitas
- [ ] T070 [US1] Implementar proposta e confirmação do mapeamento de colunas em `src/modules/imports/application/column-mapping.ts` (FR-027)
- [ ] T071 [US1] Implementar a gravação do estágio `ImportRow` em `src/modules/imports/infra/staging-repository.ts`
  - ✓ `rawData` e `parsedData` por linha; **nenhuma escrita em tabela de resultado** (FR-051, R-007)
- [ ] T072 [US1] Implementar as validações estruturais em `src/modules/imports/domain/structural-validations.ts`
  - ✓ estudante sem nome, código da turma ausente, turma ausente, coluna de habilidade ausente → `ERROR`
- [ ] T073 [US1] Implementar as validações pedagógicas em `src/modules/imports/domain/pedagogical-validations.ts`
  - ✓ avaliado × nível, avaliado sem resultados, não avaliado com resultados, habilidade fora do catálogo → `WARNING`
- [ ] T074 [US1] Implementar a detecção de colisão de chave em `src/modules/imports/domain/key-collision.ts`
  - ✓ mesma chave no arquivo e na avaliação → `ERROR` (FR-147, FR-148); mesmo nome em turma diferente → `WARNING` (FR-151)
- [ ] T075 [US1] Implementar a reconciliação com a base cadastral em `src/modules/imports/application/reconcile-students.ts`
  - ✓ ordem código → assistida → não cadastrado (FR-171); nunca vincula por nome sozinho (FR-142)
- [ ] T076 [US1] Implementar o tratamento de código único no arquivo em `src/modules/imports/application/resolve-by-code.ts`
  - ✓ desconhecido → `ERROR` sem criar estudante (FR-139); de outra escola → `WARNING` (FR-140); repetido no arquivo → `ERROR` (FR-152)
- [ ] T077 [US1] Implementar a vinculação assistida e a criação sob demanda em `src/modules/imports/application/assisted-link.ts`
  - ✓ sugere candidatos da mesma escola; criação só por ação explícita do usuário (FR-141, FR-143, FR-172)
- [ ] T078 [US1] Implementar a detecção de cadastrado ausente do arquivo em `src/modules/imports/application/detect-absent-students.ts`
  - ✓ `REGISTERED_STUDENT_ABSENT` como `WARNING`, com a lista (FR-173)
- [ ] T079 [US1] Implementar o repositório de `ImportIssue` com o catálogo de 19 códigos em `src/modules/imports/infra/issue-repository.ts`
  - ✓ cada ocorrência traz linha, coluna, código, severidade, mensagem em pt-BR e valor original (FR-043)
- [ ] T080 [US1] Implementar a API de pré-visualização em `src/app/api/imports/[id]/preview/route.ts`
  - ✓ contagens de FR-049; amostra com valor original ao lado do interpretado (FR-050); lê só do estágio
- [ ] T081 [US1] Implementar a interface do fluxo de importação em `src/app/(app)/importacoes/`
  - ✓ os seis passos de FR-019; cancelamento sem efeito colateral (FR-052)
- [ ] T082 [US1] Implementar a confirmação transacional em `src/modules/imports/application/confirm-import.ts`
...
- [ ] T083 [US1] Implementar o rollback e a transição para `FAILED` em `src/modules/imports/application/confirm-import.ts`
  - ✓ falha em qualquer ponto não deixa estado parcial
- [ ] T084 [US1] Implementar a apuração e persistência do denominador de referência em `src/modules/skills/application/resolve-reference-items.ts`
  - ✓ derivado dos dados, nunca constante; recalculado a cada importação e exclusão (FR-155, FR-161)
- [ ] T085 [US1] Implementar o histórico de importações em `src/modules/imports/application/list-imports.ts` e `src/app/(app)/importacoes/page.tsx`
  - ✓ todos os campos de FR-115
- [ ] T086 [US1] Implementar a exclusão de importação (ADMIN) em `src/modules/imports/application/delete-import.ts`
  - ✓ remove resultados, recalcula denominadores, preserva `Import`, `ImportIssue` e `AuditLog` (FR-118)
- [ ] T087 [US1] Teste de integração do pipeline completo em `tests/integration/import-pipeline.test.ts`
  - ✓ pré-visualização não grava resultado; confirmação é atômica; falha reverte tudo
- [ ] T088 [US1] Teste de integração da reimportação bloqueada em `tests/integration/reimport.test.ts`
...
- [ ] T089 [P] [US2] Implementar `calculateStudentPerformance` em `src/modules/analytics/domain/student-performance.ts`
  - ✓ retorna a fração, não o percentual; ignora nulos sem contá-los como zero; devolve `null` se não há resultado (FR-056)
- [ ] T090 [P] [US2] Implementar `calculateSkillPerformance` em `src/modules/analytics/domain/skill-performance.ts`
  - ✓ considera **apenas** `avaliado === true` (FR-059); absorve denominadores divergentes (FR-157)
- [ ] T091 [P] [US2] Implementar `calculateParticipationRate` em `src/modules/analytics/domain/participation.ts`
  - ✓ denominador é **todo** registro importado (FR-060, FR-061)
- [ ] T092 [P] [US2] Implementar `classifyAnalyticalSkillResult` em `src/modules/analytics/domain/classify.ts`
  - ✓ limites vindos de `AnalyticalSettings`; comparação sem arredondamento prévio; **sem qualquer relação com `LearningLevel`** (FR-112)
- [ ] T093 [P] [US2] Implementar `rankSkillsByFragility` em `src/modules/analytics/domain/rank-skills.ts`
  - ✓ quatro critérios de FR-072; desempate em três chaves, ordem estável entre execuções (R-011)
- [ ] T094 [US2] Testes unitários dos cálculos em `tests/unit/domain/analytics.test.ts`
  - ✓ exemplos do PRD §8 (15/22) e §9 (37/60 = 61,67%); fronteiras exatas em 60 e 80; empates de ranking
- [ ] T095 [US2] Implementar a distribuição por nível de aprendizagem em `src/modules/analytics/application/level-distribution.ts`
  - ✓ denominador **somente avaliados**; não avaliados nunca contados como Defasagem (FR-062)
- [ ] T096 [US2] Implementar as contagens de habilidades em fragilidade e em atenção por estudante em `src/modules/analytics/application/student-bands.ts`
- [ ] T097 [US2] Implementar as consultas agregadas SQL em `src/modules/analytics/infra/aggregate-queries.ts`
...
- [ ] T098 [US2] Implementar o escopo por escola nas agregações em `src/modules/analytics/infra/aggregate-queries.ts`
  - ✓ `school_id = ANY($allowedSchoolIds)`, derivado no servidor (FR-006)
- [ ] T099 [US2] Garantir que toda resposta de indicador carrega `{ acertos, itens }` junto do percentual
  - ✓ rastreabilidade de FR-064 disponível à interface
- [ ] T100 [US2] Testes de integração das agregações em `tests/integration/aggregations.test.ts`
...
- [ ] T101 [P] [US2] Implementar os componentes de card de indicador em `src/components/data/indicator-card.tsx`
  - ✓ exibe numerador e denominador; ausência de dado nunca aparece como `0%` (FR-065)
- [ ] T102 [US2] Implementar o painel de participação em `src/app/(app)/avaliacoes/[assessmentId]/page.tsx`
  - ✓ importados, avaliados, não avaliados e taxa (FR-066)
- [ ] T103 [US2] Implementar a distribuição por nível com gráfico Recharts em `src/components/charts/level-distribution.tsx`
  - ✓ quantidade e percentual de Adequado, Intermediário e Defasagem sobre os avaliados (FR-068)
- [ ] T104 [US2] Implementar o percentual geral de acertos do recorte em `src/app/(app)/avaliacoes/[assessmentId]/page.tsx` (FR-067)
- [ ] T105 [US2] Implementar o ranking H01:H12 com TanStack Table em `src/components/data/skill-ranking-table.tsx`
...
- [ ] T106 [US2] Implementar a habilidade mais frágil e a de melhor desempenho em `src/app/(app)/avaliacoes/[assessmentId]/page.tsx` (FR-069)
- [ ] T107 [US2] Implementar o ranking de turmas em `src/components/data/class-ranking-table.tsx`
...
- [ ] T108 [US2] Implementar os indicadores da escola em `src/app/(app)/escolas/[schoolId]/page.tsx`
  - ✓ turmas, estudantes, participação e percentual geral (FR-074)
- [ ] T109 [US2] Implementar a distribuição por nível da escola em `src/app/(app)/escolas/[schoolId]/page.tsx` (FR-075)
- [ ] T110 [US2] Implementar os rankings de habilidades e de turmas da escola em `src/app/(app)/escolas/[schoolId]/page.tsx` (FR-075)
- [ ] T111 [US2] Implementar as listas de estudantes em Defasagem, em Intermediário e não avaliados em `src/components/data/priority-students-list.tsx` (FR-076)
  - ✓ respeita a autorização para dados nominais
- [ ] T112 [US2] Teste E2E do escopo de escola em `tests/e2e/school-scope.spec.ts`
...
- [ ] T120 [US5] Implementar o cabeçalho da habilidade em `src/app/(app)/habilidades/[skillId]/page.tsx`
  - ✓ código curto, código pedagógico e descrição (FR-083)
- [ ] T121 [US5] Implementar os totais da habilidade no recorte em `src/app/(app)/habilidades/[skillId]/page.tsx` (FR-084)
  - ✓ quantidade de itens vem de `AssessmentSkill.referenceItems`, jamais de constante
- [ ] T122 [US5] Implementar a distribuição `0/n` … `n/n` em `src/components/charts/skill-distribution.tsx`
  - ✓ soma exatamente o total de avaliados com resultado (FR-085)
- [ ] T123 [US5] Implementar a listagem de registros com denominador divergente, à parte da distribuição, em `src/components/data/divergent-denominators-list.tsx`
  - ✓ informa quantos ficaram fora e por quê (FR-158, FR-159)
- [ ] T124 [US5] Implementar o ranking de turmas na habilidade em `src/components/data/class-ranking-table.tsx` (FR-086)
- [ ] T125 [US5] Implementar a lista de estudantes com maior dificuldade em `src/components/data/struggling-students-list.tsx` (FR-087)
...
- [ ] T137 [US6] Implementar o esquema Zod das quinze dimensões em `src/modules/analytics/schemas/filters.ts` (FR-098)
- [ ] T138 [US6] Implementar a tradução de filtros para as consultas agregadas em `src/modules/analytics/infra/filter-builder.ts`
  - ✓ `schoolId` do cliente é **filtro dentro** de `allowedSchoolIds`, nunca autorização
- [ ] T139 [US6] Implementar o componente de filtros em `src/components/data/filter-bar.tsx`
  - ✓ filtros ativos legíveis e removíveis (FR-100)
- [ ] T140 [US6] Propagar os filtros a todos os indicadores, rankings, gráficos e listas em `src/modules/analytics/application/apply-filters.ts` (FR-099)
- [ ] T141 [US6] Implementar o estado vazio de filtro sem resultados em `src/components/data/empty-state.tsx` (FR-099)
  - ✓ informa ausência de registros; **não exibe indicadores zerados**
- [ ] T142 [US6] Teste de integração dos filtros em `tests/integration/filters.test.ts`
...
- [ ] T171 Gerar a fixture anonimizada em `tests/fixtures/resultados-referencia.csv`
...
- [ ] T172 [P] Gerar a fixture de nominata em `tests/fixtures/nominata-referencia.csv`
- [ ] T173 [P] Gerar as fixtures de casos inválidos em `tests/fixtures/casos-invalidos/`
  - ✓ uma por código do catálogo de 19 inconsistências
- [ ] T174 Consolidar os testes unitários de parser e cálculos em `tests/unit/` conforme a tabela de cobertura de [contracts/domain-functions.md](contracts/domain-functions.md)
- [ ] T175 Consolidar os testes de integração de importação, autorização e agregações em `tests/integration/`
- [ ] T176 Escrever o teste de regressão do Princípio X em `tests/regression/reference-file.test.ts`
  - ✓ 111 / 106 / 5 / 4 / 12; distribuição 96 / 7 / 3; ranking do PRD §38.1 com tolerância de 0,01 p.p.
- [ ] T177 Verificar a integridade da fixture por SHA-256 do conteúdo numérico em `tests/regression/fixture-integrity.test.ts`
...
- [ ] T193 Escrever o teste de responsividade em `tests/e2e/responsive.spec.ts` (FR-123, SC — Const. VIII)
  - ✓ percorre dashboard geral, turma, ficha e mapa de calor em 375 px, 768 px e 1280 px sem rolagem horizontal da página
- [ ] T194 Escrever o teste da permissão de dados nominais em `tests/integration/nominal-permission.test.ts` (FR-007, FR-007a)
...
- [ ] T179 Configurar o portão de integração em `.github/workflows/ci.yml`
...
- [ ] T181 [P] Escrever o `README.md` com visão geral, instalação e configuração
  - ✓ inclui a nota sobre o registro oficial do SheetJS (R-005), para não ser "corrigida" por engano
- [ ] T182 [P] Documentar a preparação do banco, migrations e seed no `README.md`
- [ ] T183 [P] Documentar o fluxo de importação passo a passo no `README.md`
- [ ] T184 [P] Documentar a execução de testes, E2E, regressão e build no `README.md`
- [ ] T185 [P] Documentar em `README.md` a política de dados: o arquivo real não é versionado e a fixture é anonimizada
